...
func TestAuth(t *testing.T) {
	...
	if exp, got := len(claims.Roles), len(parsedClaims.Roles); exp != got {
		...
		t.Fatalf("\t%s\tTest %d:\t Should be have the expected number of roles %v", failed, testID, err)
	}
	t.Logf("\t%s\tTest %d:\t Should be have the expected number of roles", success, testID)

	if exp, got := claims.Roles[0], parsedClaims.Roles[0]; exp != got {
		t.Logf("\t\tTest %d:\texp: %s", testID, exp)
		t.Logf("\t\tTest %d:\tgot: %s", testID, got)
		t.Fatalf("\t%s\tTest %d:\t Should be have the expected roles %v", failed, testID, err)
	}
	t.Logf("\t%s\tTest %d:\t Should be have the expected roles", success, testID)
}
